            ...
            {visibleMembers.map((member) => {
              ...
              return (
                <CrewMemberRow
                  key={member.userId}
                  member={member}
                  action={member.role === "OWNER" && isFamilyOwner ? (
                    <Link
                      aria-label="크루 설정"
...
function GuestHome({ authError, isLoading = false }: { authError?: string | null; isLoading?: boolean }) {
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="guest-background-enter absolute inset-0 -z-20 bg-cover bg-[center_30%]"
        style={{
      ...
      <div className="mx-auto grid min-h-0 w-full max-w-screen-2xl items-end gap-7 pb-4 sm:pb-0 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,28rem)] lg:gap-16 lg:px-12">
        <section className="max-w-2xl text-white">
          ...
          <p className="guest-copy-enter mt-4 max-w-lg text-sm font-normal leading-6 text-white/85 sm:text-base">
...
function FamilyLocationMap({
  ...
  const pinSignature = pins
    .map(
      ({ location, member }) =>
        `${member.userId}:${location.latitude.toFixed(6)},${location.longitude.toFixed(6)}`
    )
    .join("|");
  ...
  useEffect(() => {
    ...
    if (existingMap) {
      ...
      setProjectedPositions(projectKakaoPinPositions(existingMap, pinsRef.current));
      return () => {
        active = false;
      };
    }
    ...
    async function initializeMap() {
      ...
      try {
        ...
        const center = getAverageLocation(currentPins.map((pin) => pin.location));
        ...
        kakao.event.addListener(map, "idle", () => {
          const latestPins = pinsRef.current;

          if (latestPins.length > 0) {
            setProjectedPositions(projectKakaoPinPositions(map, latestPins));
          }
        });
        ...
        window.setTimeout(() => {
          if (active) {
            map.relayout();
            setProjectedPositions(projectKakaoPinPositions(map, pinsRef.current));
            setMapStatus("READY");
          }
        }, 100);
      } catch {
