...
- 운영 배포 대상은 Firebase Hosting 하나로 유지한다.
- `npm run test:release`는 lint, unit, UI 피드백, Firebase Rules, 운영 빌드와 산출물 검사를 순서대로 실행한다.
- GitHub Actions는 배포 뒤 관리자·초대 링크를 포함한 SPA 경로와 배포 커밋, PWA 공개 자산, 캐시·보안 헤더를 검사한다.
- 해시가 붙은 `/assets/**`는 장기 캐시하고, `manifest.webmanifest`와 `sw.js`는 새 배포 감지를 위해 캐시하지 않는다.
