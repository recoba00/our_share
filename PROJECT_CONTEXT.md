...
사용자에게 노출되는 서비스 용어는 `가족` 대신 `그룹`을 사용한다. 그룹은 가족, 친구 모임, 룸메이트, 소규모 팀까지 확장할 수 있는 상위 개념이다.

현재 MVP의 Firebase 컬렉션과 코드 내부 식별자는 기존 데이터 호환을 위해 `families`, `familyId`, `familyMembers`를 유지한다. 이 이름은 사용자 화면에 노출하지 않는 내부 호환 계층이다.

다중 그룹 확장 방향:

- 한 사용자는 여러 그룹의 멤버가 될 수 있다.
- 그룹마다 OWNER와 구성원 역할을 별도로 가진다.
...
- 마지막 선택 그룹은 로컬 저장소에 보관하고, 그룹 전환 UI는 헤더 또는 홈 상단에 배치한다.
