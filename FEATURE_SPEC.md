...
사용자 화면에서는 `가족`·`그룹`을 `크루`, `오너`를 `크루장`, `그룹원`·`구성원`을 `멤버`로 표시한다. Firebase 내부 컬렉션과 필드명은 데이터 호환을 위해 기존 이름을 유지한다.
...
- 크루장: 멤버 역할 변경, 멤버 삭제, 초대, 크루 수정·삭제
- 부크루장: 초대 링크 공유
- 멤버: 크루 기능 이용

크루장 승계는 확인 다이얼로그를 거치며, 기존 크루장은 자동으로 부크루장이 된다.
