...
- 초대 링크 로그인 후 본인 멤버십 문서가 아직 없는 상태에서 존재 여부를 확인할 때 권한 오류가 발생하지 않도록 멤버십 ID 기반 본인 조회 규칙과 테스트를 보완했다.
- GitHub Actions 자동배포에 Firestore 보안 규칙 배포 단계를 추가해 초대 가입 권한 수정이 Hosting과 함께 실제 Firebase에 반영되도록 했다.
